/**
 * @file MarketplaceItemController.cc
 * @brief MarketplaceItem控制器实现
 *
...
#include "MarketplaceItemController.h"

namespace community {
namespace controllers {

// ========== 构造函数 ==========
MarketplaceItemController::MarketplaceItemController() {
    LOG_INFO << "MarketplaceItemController 已创建（Drogon 自动实例化）";
}

// ========== 获取服务实例 ==========
std::shared_ptr<services::MarketplaceItemService> MarketplaceItemController::getService() const {
    return services::MarketplaceItemService::getInstance();
}

// ========== GET /api/marketplace/items ==========
void MarketplaceItemController::getList(
    ...
    std::function<void(const drogon::HttpResponsePtr&)>&& callback)
{
    ...
    std::string filter1;
    try {
        auto s = req->getParameter("exchange_type");
    ...
    int total = 0;
    auto dataList = svc->getMarketplaceItemsByPage(page, pageSize, total, keyword, filter1, filter2);
...
void MarketplaceItemController::getById(
    const drogon::HttpRequestPtr& /*req*/,
    std::function<void(const drogon::HttpResponsePtr&)>&& callback,
    int id)
{
    ...
    auto item = svc->getMarketplaceItemById(id);
    if (!item) {
...
void MarketplaceItemController::create(
    const drogon::HttpRequestPtr& req,
    std::function<void(const drogon::HttpResponsePtr&)>&& callback)
{
    ...
    auto obj = models::MarketplaceItem::fromJson(*jsonBody);
    ...
    int newId = svc->createMarketplaceItem(obj);
    if (newId <= 0) {
...
void MarketplaceItemController::update(
    const drogon::HttpRequestPtr& req,
    std::function<void(const drogon::HttpResponsePtr&)>&& callback,
    int id)
{
    ...
    bool ok = svc->updateMarketplaceItem(obj);
    if (!ok) {
...
void MarketplaceItemController::remove(
    const drogon::HttpRequestPtr& /*req*/,
    std::function<void(const drogon::HttpResponsePtr&)>&& callback,
    int id)
{
    ...
    bool ok = svc->deleteMarketplaceItem(id);
    if (!ok) {
        callback(ResponseUtil::error(RespCode::NOT_FOUND, "记录不存在"));
        return;
    }
